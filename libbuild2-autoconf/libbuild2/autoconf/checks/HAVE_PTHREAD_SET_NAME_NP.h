...
/* Check for the pthread_set_name_np() function which sets the name of a
 * thread.
 *
 * Since FreeBSD 5.1 and OpenBSD 2.4 (December 1998).
 *
 * Note that this check is identical to HAVE_PTHREAD_NP_H because
 * pthread_set_name_np() is declared in pthread_np.h so be sure to replicate
 * any changes to HAVE_PTHREAD_NP_H.h.
 */
